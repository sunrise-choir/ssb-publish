...
#[cfg(test)]
mod tests {
    use crate::{Contact, publish, Content};
    ...
    fn it_works() {
        ...
        let contact = Contact{
            contact: Multikey::from_legacy(b"@9Zf0se86PotjNqaOt9ue8BNBLkGVLQcLNDw/pRQHY3U=.ed25519").unwrap().0,
            following: true,
            blocking: false,
        };
        let content = Content::Plain(contact);
        let msg = publish(content, None, keypair.public.as_bytes() , keypair.secret.as_bytes() , 0.0).unwrap();

        let is_valid = ssb_validate::validate_hash_chain(&msg, None).is_ok();
        let is_verified = ssb_verify_signatures::verify(&msg).is_ok();
